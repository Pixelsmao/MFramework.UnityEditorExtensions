...
namespace MFramework.EditorExtensions
{
    ...
    public class EditorWindowBaseAbandon : EditorWindow
    {
        ...
        {
            GUIStyle labelStyle = EditorGUIStyles.AlignmentLabelStyle(textAnchor);
        ...
        {
            var labelStyle = EditorGUIStyles.AlignmentLabelStyle(TextAnchor.MiddleCenter);
        ...
        protected void DrawAnimFoldedGroup(string foldedTitle, Action drawContentAction, bool expand = false)
        {
            ...
            if (GUILayout.Button(foldGroup.AnimBool.target ? foldGroup.UnfoldContent : foldGroup.FoldContent,
                    EditorGUIStyles.AnimFadeGroupButtonStyle))
            {
                // 关闭所有其他折叠组
        ...
        {
            var remainingBuggedEditors = FindObjectsOfType<EditorWindowBaseAbandon>();
